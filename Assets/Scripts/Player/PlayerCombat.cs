...
public class PlayerCombat : MonoBehaviour, IDamagable<DamageObject> {
	...
	public Weapon currentWeapon; //the current weapon the player is holding
	...
	IEnumerator destroyCurrentWeapon(float delay){
		...
		{ 
		if(currentWeapon.degenerateType == DEGENERATETYPE.DEGENERATEONUSE && currentWeapon != null) GlobalAudioPlayer.PlaySFX(currentWeapon.breakSound);
		Destroy(currentWeapon.playerHandPrefab);
		currentWeapon.BreakWeapon();
		currentWeapon = null;
		
		UI.PunchIcon.sprite = UI.ReplacePunchImgs[0];
		}
	}
	...
	public void CheckForHit() {
		...
		if (currentWeapon != null)
		{
			if (currentWeapon.weaponName != null)
			{
	...
	public void ItemInRange(GameObject item){
		itemInRange = item;
		UI.PunchIcon.sprite = UI.ReplacePunchImgs[1];
	}
	...
	public void UpdatePunchIco()
	{
		//Change punch sprites according to weapon
		if (currentWeapon != null)
		{
			if (currentWeapon.weaponName != null)
			{
				...
				{
					case "BaseballBat":
						UI.PunchIcon.sprite = UI.ReplacePunchImgs[2];
						break;
					case "Sword":
						UI.PunchIcon.sprite = UI.ReplacePunchImgs[3];
						break;
					case "Knife":
						UI.PunchIcon.sprite = UI.ReplacePunchImgs[4];
						break;
					case "Gun":
						UI.PunchIcon.sprite = UI.ReplacePunchImgs[5];
						break;
					default:
						UI.PunchIcon.sprite = UI.ReplacePunchImgs[0];
						break;
	...
	void Death(){
		...
		//GA Event
		//MyAnalytics.instance.ScreenEvent("LoseScreen");
	}
	...
	IEnumerator ReStartLevel(){
		...
		if (UI != null){
			...
			//GA Event
			//MyAnalytics.instance.LevelFailedEvent(GlobalGameSettings.currentLevelId.ToString());
		}
	}
}