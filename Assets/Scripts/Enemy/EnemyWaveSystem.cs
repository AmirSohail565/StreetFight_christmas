...
public class EnemyWaveSystem : MonoBehaviour {
		...
		if(EnemyWaves.Length > currentWave){
			...
			if(EnemyWaves[currentWave].waveComplete()){
				...
				if(!allWavesCompleted()){ 
					...
					{
						InvokeRepeating("checkBossDeath", 1, 0.5f);
					}
				}
			}
		}
	...
	IEnumerator LevelComplete(){
		...
		//Save level completed progress
		if (PlayerPrefs.GetInt("LevelsCompleted", 0) <= GlobalGameSettings.currentLevelId + 1) 
		{
			PlayerPrefs.SetInt("LevelsCompleted", GlobalGameSettings.currentLevelId + 1);
		}

		//GA Event
		//MyAnalytics.instance.LevelPassEvent(GlobalGameSettings.currentLevelId.ToString());
